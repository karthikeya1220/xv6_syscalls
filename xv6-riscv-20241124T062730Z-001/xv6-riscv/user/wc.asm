...
wc(int fd, char *name)
{
  ...
  while((n = read(fd, buf, sizeof(buf))) > 0){
    ...
    1032:	492d8d93          	addi	s11,s11,1170 # 24c0 <buf>
    for(i=0; i<n; i++){
      c++;
      if(buf[i] == '\n')
    1036:	4aa9                	li	s5,10
  ...
  while((n = read(fd, buf, sizeof(buf))) > 0){
    ...
    1086:	43e48493          	addi	s1,s1,1086 # 24c0 <buf>
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    ...
    1866:	c4e7b783          	ld	a5,-946(a5) # 24b0 <freep>
    ...
    188e:	00e6ea63          	bltu	a3,a4,18a2 <free+0x4a>
{
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    18cc:	00001717          	auipc	a4,0x1
    18d0:	bef73223          	sd	a5,-1052(a4) # 24b0 <freep>
}
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    18f6:	00001517          	auipc	a0,0x1
    18fa:	bba53503          	ld	a0,-1094(a0) # 24b0 <freep>
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    if(p == freep)
    1926:	00001917          	auipc	s2,0x1
    192a:	b8a90913          	addi	s2,s2,-1142 # 24b0 <freep>
  if(p == (char*)-1)
    192e:	5afd                	li	s5,-1
    ...
    193e:	d8678793          	addi	a5,a5,-634 # 26c0 <base>
    1942:	00001717          	auipc	a4,0x1
    1946:	b6f73723          	sd	a5,-1170(a4) # 24b0 <freep>
  ...
  if(p == (char*)-1)
    ...
    19ac:	0137a423          	sw	s3,8(a5)
      freep = prevp;
    19b0:	00001717          	auipc	a4,0x1
    19b4:	b0a73023          	sd	a0,-1280(a4) # 24b0 <freep>
      return (void*)(p + 1);
